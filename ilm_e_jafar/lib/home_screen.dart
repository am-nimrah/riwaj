...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadAllNames() async {
    ...
    try {
      ...
      setState(() => _errorMessage = 'Error loading names: ${e.toString()}\nPlease try again later.');

      // Show a more detailed error dialog in debug mode
      if (!kReleaseMode) {
  ...
  Future<Records> deserializeNames() async {
    const url = 'https://raw.githubusercontent.com/am-nimrah/riwaj/main/asma_ul_hasna.json';

    try {
      final response = await http.get(Uri.parse(url));

      if (response.statusCode != 200) {
        throw Exception('Failed to load names data: ${response.statusCode}');
      }

      // Decode with explicit UTF-8 to prevent encoding issues
      final data = json.decode(utf8.decode(response.bodyBytes));

      // Add more robust parsing with null checks
      final recordsData = data['records'];
      if (recordsData == null) {
        throw Exception('Invalid JSON format: missing "records" key');
      }

      final recordsJson = recordsData['record'] is List
          ? recordsData['record'] as List
          : [];

      final records = recordsJson.map((jsonRecord) {
        try {
          return Record(
            sNo: jsonRecord['SNo']?.toString() ?? '',
            english: jsonRecord['English']?.toString() ?? '',
            arabic: jsonRecord['Arabic']?.toString() ?? '',
            value: jsonRecord['Value']?.toString() ?? '',
            type: jsonRecord['Type']?.toString() ?? '',
            meaning: jsonRecord['Meaning']?.toString() ?? '',
          );
        } catch (e) {
          debugPrint('Error parsing record: $e');
          return null;
        }
      }).where((record) => record != null && record.value.isNotEmpty).cast<Record>().toList();

      return Records(records: records);
    } catch (e) {
      debugPrint('JSON error: $e');
      throw Exception('Failed to parse names data: ${e.toString()}');
    }
  }
  ...
  Widget _buildTableView() {
    var filteredRecords = _includeNearMatches
        ? _result.records.where((record) => record.commulativeDigits != record.inputDigits).toList()
        : _result.records.where((record) => _selectedFilter == 'All' || record.category == _selectedFilter).toList();
              ...
              children: [
                ...
                if (isSingleCategory) ...[
                  ...
                  if (record.componentNames != null && record.componentNames!.isNotEmpty) ...[
                    const SizedBox(height: 8),
                    Text('Components: ${record.commulativeDigits}', style: const TextStyle(fontWeight: FontWeight.bold)),
                    ...record.componentNames!.map((component) => Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            '• ${component.arabic} (${component.value})',
                            textDirection: TextDirection.rtl,
